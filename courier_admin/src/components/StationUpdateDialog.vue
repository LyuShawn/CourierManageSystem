<template>
  <el-dialog v-model="$store.state.HomeModule.dialogFormVisible" title="请输入修改信息">
    <el-form :model="form" :label-width="formLabelWidth">
	  ...
	  <el-form-item label="负责人身份证号" :label-width="formLabelWidth">
	    <el-input v-model="form.principal_identify_id" autocomplete="off" />
	  </el-form-item>
	  <el-form-item label="负责人姓名" :label-width="formLabelWidth">
	    <el-input v-model="form.principal_name" autocomplete="off" />
	  </el-form-item>
...
let closeDialog=(num:number)=>{
	...
	if(num==1){
		...
		link(url.stationlist+"/"+store.state.HomeModule.uplistData.id,"PUT",data).then((ok:any)=>{
			console.log(ok)
		})
	}
